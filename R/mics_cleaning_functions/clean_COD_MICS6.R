...
clean_COD_MICS6 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/DRCongo MICS6 SPSS Datafiles/DRCongo MICS6 SPSS Datafiles/wm.sav")
  hh <- read_sav("data/mics/raw_data/DRCongo MICS6 SPSS Datafiles/DRCongo MICS6 SPSS Datafiles/hh.sav")
  bh <- read_sav("data/mics/raw_data/DRCongo MICS6 SPSS Datafiles/DRCongo MICS6 SPSS Datafiles/bh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/COD_MICS6_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/COD_MICS6_hh_dict.xlsx")
  write_xlsx(bh.dict, "data/mics/data_dictionaries/COD_MICS6_bh_dict.xlsx")
  ...
  data$relig <- ifelse(data$relig=="non reponse", NA, data$relig)
  data$relig <- ifelse(data$relig=="animisme/religion traditionnelle africaine", "animist", data$relig)
  data$relig <- ifelse(data$relig=="autre religion", "other religion", data$relig)
  data$relig <- ifelse(data$relig=="eglise de reveil", "pentecostal", data$relig)
  data$relig <- ifelse(data$relig=="protestante", "protestant", data$relig)
  data$relig <- ifelse(data$relig=="temoin de jehovah", "jehovah's witness", data$relig)
  data$relig <- ifelse(data$relig=="armee du salut", "salvation army", data$relig)
  data$relig <- ifelse(data$relig=="musulmane", "muslim", data$relig)
  data$relig <- ifelse(data$relig=="catholique", "catholic", data$relig)
  data$relig <- ifelse(data$relig=="kimbanguiste", "kimbanguism", data$relig)
  data$relig <- ifelse(data$relig=="sans religion", "no religion", data$relig)
  table(data$relig, useNA = "always")
  
  # Ethnicity
  data$ethn <- tolower(as.character(as_factor(mics$HC2)))
  ...
  data$prev_reg <- ifelse(data$prev_reg=="en dehors de la rdc", "international", data$prev_reg)
