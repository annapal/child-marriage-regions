...
clean_VNM_MICS3 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Vietnam_Datasets/Vietnam MICS 2006 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Vietnam_Datasets/Vietnam MICS 2006 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/VNM_MICS3_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/VNM_MICS3_hh_dict.xlsx")
  
  # Merge womens and household survey
  mics <- merge(wm, hh, by=c("diaban", "hh2"), suffixes = c("", ".hh"))
  ...
  # Cluster number
  data$clust <- as.numeric(mics$diaban)
  summary(data$clust)
  ...
  # Women's line number
  data$line <- as.numeric(mics$matv)
  summary(data$line)
  ...
  summary(data$int_m)
  data$int_y <- as.numeric(mics$hh5y)
  summary(data$int_y)
  
  # Primary sampling unit
  data$psu <- as.numeric(mics$diaban)
  
  # Sub-national region names
  data$region <- tolower(as.character(as_factor(mics$hh7)))
  data$prov <- tolower(as.character(as_factor(mics$tinh)))
  data$dist <- tolower(as.character(as_factor(mics$huyen)))
  ...
  # Birth month
  data$birth_m <- as.numeric(mics$wm8b)
  data[data$birth_m%in%c(97,98,99), "birth_m"] <- NA
  summary(data$birth_m)
  
  # Birth year
  data$birth_y <- as.numeric(mics$wm8c)
  ...
  # Ethnicity
  data$ethn <- tolower(as.character(as_factor(mics$ethnic)))
  ...
  # Marriage month
  data$mar_m <- as.numeric(mics$ma6b1)
  data[data$mar_m%in%c(97,98,99), "mar_m"] <- NA
  summary(data$mar_m)
  
  # Marriage year
  data$mar_y <- as.numeric(mics$ma6b2)
