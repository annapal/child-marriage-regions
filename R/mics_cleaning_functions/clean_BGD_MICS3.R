...
clean_BGD_MICS3 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Bangladesh 2006 MICS_Datasets/Bangladesh MICS 2006 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Bangladesh 2006 MICS_Datasets/Bangladesh MICS 2006 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/BGD_MICS3_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/BGD_MICS3_hh_dict.xlsx")
  ...
  data$dist <- tolower(as.character(as_factor(mics$HH7B)))
  
  # Rural-urban place of residence
  data$res <- ifelse(mics$HH6%in%c(1,5), "rural", ifelse(mics$HH6%in%c(2,3,4), "urban", NA))
  ...
  data$relig <- ifelse(data$relig%in%c("no response", "missing"), NA, data$relig)
  table(data$relig, useNA = "always")
  
  # Ethnicity
  data$ethn <- tolower(as.character(as_factor(mics$HC1C)))
  data$ethn <- ifelse(data$ethn%in%c("no response", "missing"), NA, data$ethn)
