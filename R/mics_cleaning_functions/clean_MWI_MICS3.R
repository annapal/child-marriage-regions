...
clean_MWI_MICS3 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Malawi MICS 2006 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Malawi MICS 2006 SPSS Datasets/hh.sav")
  bh <- read_sav("data/mics/raw_data/Malawi MICS 2006 SPSS Datasets/bh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/MWI_MICS3_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/MWI_MICS3_hh_dict.xlsx")
  write_xlsx(bh.dict, "data/mics/data_dictionaries/MWI_MICS3_bh_dict.xlsx")
  ...
  summary(data$psu)
  data$strata <- as.numeric(mics$HHSTRAT)
  summary(data$strata)
  
  # Sub-national region names
  data$region <- tolower(as.character(as_factor(mics$HHDIS)))
  data$prov <- NA
  data$dist <- NA
  ...
  data$relig <- ifelse(data$relig=="other christian", "other christian religion", data$relig)
  data$relig <- ifelse(data$relig=="ccap", "church of central africa presbyterian", data$relig)
  table(data$relig, useNA = "always")
  
  # Ethnicity
  data$ethn <- tolower(as.character(as_factor(mics$HC1B)))
