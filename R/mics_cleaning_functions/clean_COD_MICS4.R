...
clean_COD_MICS4 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Democratic Republic of Congo_MICS4_Datasets/Democratic Republic of Congo MICS 2010 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Democratic Republic of Congo_MICS4_Datasets/Democratic Republic of Congo MICS 2010 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/COD_MICS4_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/COD_MICS4_hh_dict.xlsx")
  ...
  data$relig <- ifelse(data$relig=="animisme/ religions traditionnelles afric.", "animism", data$relig)
  ...
  data$relig <- ifelse(data$relig=="autres chretiens independants", "other christian religion", data$relig)
  data$relig <- ifelse(data$relig=="catholique", "catholic", data$relig)
  data$relig <- ifelse(data$relig=="eglises de reveil", "pentecostal", data$relig)
  data$relig <- ifelse(data$relig=="kimbanguiste", "kimbanguist", data$relig)
  data$relig <- ifelse(data$relig=="musulman", "muslim", data$relig)
  data$relig <- ifelse(data$relig=="sans religion", "no religion", data$relig)
  data$relig <- ifelse(data$relig=="temoins de jehovah", "jehovah's witness", data$relig)
  table(data$relig, useNA = "always")
  
  # Ethnicity
  data$ethn <- tolower(as.character(as_factor(mics$HC1C)))
  data$ethn <- ifelse(data$ethn=="manquant", NA, data$ethn)
