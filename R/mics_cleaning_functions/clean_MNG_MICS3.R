...
clean_MNG_MICS3 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Mongolia MICS 2005 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Mongolia MICS 2005 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/MNG_MICS3_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/MNG_MICS3_hh_dict.xlsx")
  ...
  data$prov <- tolower(as.character(as_factor(mics$HH7w)))
  data$dist <- NA
  ...
  # Ever had sex
  data$sex_ev <- ifelse(mics$MA8A==99, NA, 
                        ifelse(mics$MA8A==0, 0, 
                               ifelse(mics$MA8A>0, 1, NA)))
  table(data$sex_ev, useNA="always")
  
  # Age at first sexual intercourse
  data$sex_age <- as.numeric(mics$MA8A)
