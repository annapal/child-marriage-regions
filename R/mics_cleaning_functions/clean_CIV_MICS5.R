...
clean_CIV_MICS5 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Côte d'Ivoire_MICS5Datasets/Côte d'Ivoire 2016 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Côte d'Ivoire_MICS5Datasets/Côte d'Ivoire 2016 SPSS Datasets/hh.sav")
  bh <- read_sav("data/mics/raw_data/Côte d'Ivoire_MICS5Datasets/Côte d'Ivoire 2016 SPSS Datasets/bh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/CIV_MICS5_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/CIV_MICS5_hh_dict.xlsx")
  write_xlsx(bh.dict, "data/mics/data_dictionaries/CIV_MICS5_bh_dict.xlsx")
  ...
  data$relig <- ifelse(data$relig=="autre religion (préciser)", "other religion", data$relig)
  data$relig <- ifelse(data$relig=="autre religion chrétienne (préciser)", "other christian religion", data$relig)
  data$relig <- ifelse(data$relig=="catholique", "catholic", data$relig)
  data$relig <- ifelse(data$relig=="céleste", "celestial", data$relig)
  data$relig <- ifelse(data$relig=="evangélique", "evangelical", data$relig)
  data$relig <- ifelse(data$relig=="harriste", "harris mission", data$relig)
  ...
  data$ethn <- ifelse(data$ethn=="non déclaré/pas de réponse", NA, data$ethn)
