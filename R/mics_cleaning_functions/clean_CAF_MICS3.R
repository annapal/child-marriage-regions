...
clean_CAF_MICS3 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Central African Republic MICS 2006 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Central African Republic MICS 2006 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/CAF_MICS3_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/CAF_MICS3_hh_dict.xlsx")
  ...
  # Marriage month
  data$mar_m <- as.numeric(mics$MA7M)
  data[data$mar_m%in%c(97,98,99), "mar_m"] <- NA
  summary(data$mar_m)
  
  # Marriage year
  data$mar_y <- as.numeric(mics$MA7Y)
