...
clean_STP_MICS5 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Sao Tome and Principe_MICS5_Datasets/Sao Tome and Principe MICS 2014 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Sao Tome and Principe_MICS5_Datasets/Sao Tome and Principe MICS 2014 SPSS Datasets/hh.sav")
  bh <- read_sav("data/mics/raw_data/Sao Tome and Principe_MICS5_Datasets/Sao Tome and Principe MICS 2014 SPSS Datasets/bh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/STP_MICS5_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/STP_MICS5_hh_dict.xlsx")
  write_xlsx(bh.dict, "data/mics/data_dictionaries/STP_MICS5_bh_dict.xlsx")
  ...
  data$relig <- ifelse(data$relig=="em falta", NA, data$relig)
  data$relig <- ifelse(data$relig=="adventista", "seventh day adventist", data$relig)
  data$relig <- ifelse(data$relig=="catã³lica", "catholic", data$relig)
  data$relig <- ifelse(data$relig=="catã³lica", "catholic", data$relig)
  data$relig <- ifelse(data$relig=="nova apostã³lica", "new apostolic", data$relig)
  data$relig <- ifelse(data$relig=="outras religiãµes (especificar)", "other religion", data$relig)
  data$relig <- ifelse(data$relig=="sem religiã£o", "no religion", data$relig)
