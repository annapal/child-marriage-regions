...
clean_MNG_MICS4 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Mongolia_MICS4_Datasets/Mongolia MICS 2010 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Mongolia_MICS4_Datasets/Mongolia MICS 2010 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/MNG_MICS4_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/MNG_MICS4_hh_dict.xlsx")
  ...
  data$int_m <- as.numeric(mics$WM6_M)
  summary(data$int_m)
  data$int_y <- as.numeric(mics$WM6_Y)
  summary(data$int_y)
  ...
  data$region <- tolower(as.character(as_factor(mics$HH7A)))
  data$prov <- NA
  data$dist <- NA
  
  # Rural-urban place of residence
  data$res <- ifelse(mics$HH6==1|mics$HH6==2|mics$HH6==3, "urban", ifelse(mics$HH6==4, "rural", NA))
  ...
  # Birth month
  data$birth_m <- as.numeric(mics$WB1_M)
  data[data$birth_m%in%c(97,98,99), "birth_m"] <- NA
  summary(data$birth_m)
  
  # Birth year
  data$birth_y <- as.numeric(mics$WB1_Y)
  ...
  data$relig <- ifelse(data$relig=="dk", NA, data$relig)
  data$relig <- ifelse(data$relig=="does not hold any religion", "no religion", data$relig)
  data$relig <- ifelse(data$relig=="other", "other religion", data$relig)
  table(data$relig, useNA = "always")
  
  # Ethnicity
  data$ethn <- tolower(as.character(as_factor(mics$ethnicity)))
  data$ethn <- ifelse(data$ethn=="missing/dk", NA, data$ethn)
  ...
  # Marriage month
  data$mar_m <- as.numeric(mics$MA8_M)
  data[data$mar_m%in%c(97,98,99), "mar_m"] <- NA
  summary(data$mar_m)
  
  # Marriage year
  data$mar_y <- as.numeric(mics$MA8_Y)
