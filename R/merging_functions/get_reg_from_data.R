...
get_reg_from_data <- function(data, merge_dat) {
  ...
  # Reg
  var <- merge_dat$Reg_var[1] # Survey variable to match to Reg
  if (!is.na(var)) {
    # Add matches if applicable
  ...
  # Adm2
  var <- merge_dat$Adm2_var[1] # Survey variable to match to Adm2
  if (!is.na(var)) {
    # Add matches if applicable
  ...
  # Return data with region variables
  data
}
