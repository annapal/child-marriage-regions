...
clean_DOM_MICS5 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Dominican Republic_MICS5_Datasets/Dominican Republic MICS 2014 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Dominican Republic_MICS5_Datasets/Dominican Republic MICS 2014 SPSS Datasets/hh.sav")
  bh <- read_sav("data/mics/raw_data/Dominican Republic_MICS5_Datasets/Dominican Republic MICS 2014 SPSS Datasets/bh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/DOM_MICS5_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/DOM_MICS5_hh_dict.xlsx")
  write_xlsx(bh.dict, "data/mics/data_dictionaries/DOM_MICS5_bh_dict.xlsx")
  ...
  # Religion
  data$relig <- tolower(as.character(as_factor(mics$ethnicity)))
  data$relig <- ifelse(data$relig=="omitido/no sabe", NA, data$relig)
  data$relig <- ifelse(data$relig=="ninguna", "no religion", data$relig)
  data$relig <- ifelse(data$relig=="católica", "catholic", data$relig)
  data$relig <- ifelse(data$relig=="evangélica", "evangelical", data$relig)
  data$relig <- ifelse(data$relig=="otra religión", "other religion", data$relig)
