...
clean_TGO_MICS6 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Togo MICS6 SPSS Datasets/Togo MICS6 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Togo MICS6 SPSS Datasets/Togo MICS6 SPSS Datasets/hh.sav")
  bh <- read_sav("data/mics/raw_data/Togo MICS6 SPSS Datasets/Togo MICS6 SPSS Datasets/bh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/TGO_MICS6_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/TGO_MICS6_hh_dict.xlsx")
  write_xlsx(bh.dict, "data/mics/data_dictionaries/TGO_MICS6_bh_dict.xlsx")
  ...
  data$relig <- ifelse(data$relig=="autre religion chretienne", "other christian religion", data$relig)
  data$relig <- ifelse(data$relig=="catholique", "catholic", data$relig)
  data$relig <- ifelse(data$relig=="methodiste", "methodist", data$relig)
  data$relig <- ifelse(data$relig=="musulmane", "muslim", data$relig)
  data$relig <- ifelse(data$relig=="evangelique presbyterienne", "evangelical presbytarian", data$relig)
  data$relig <- ifelse(data$relig=="assemblee de dieu", "assemblies of god", data$relig)
  ...
  data$al_lived <- ifelse(is.na(mics$WB15)|mics$WB15==99|mics$WB15==97, NA,
                          ifelse(mics$WB15==95, 1, 0))
  table(data$al_lived, useNA = "always")
  
  # Number of years lived in city/village
  data$yrs_lived <- ifelse(is.na(mics$WB15)|(mics$WB15 %in% c(99, 97, 95)), NA,
                           as.numeric(mics$WB15))
  summary(data$yrs_lived)
  
  # Region of previous residence
  data$prev_reg <- tolower(as.character(as_factor(mics$WB17)))
  unique(data$prev_reg)
  data$prev_reg <- ifelse(data$prev_reg=="non reponse", NA, data$prev_reg)
