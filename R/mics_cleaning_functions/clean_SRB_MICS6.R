...
clean_SRB_MICS6 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Serbia MICS6 Datasets/Serbia MICS6 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Serbia MICS6 Datasets/Serbia MICS6 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/SRB_MICS6_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/SRB_MICS6_hh_dict.xlsx")
  ...
  data$relig <- ifelse(data$relig=="does not want to declare", NA, data$relig)
  table(data$relig, useNA = "always")
  
  # Ethnicity
  data$ethn <- tolower(as.character(as_factor(mics$HC2)))
  data$ethn <- ifelse(data$ethn=="does not want to declare", NA, data$ethn)
