...
clean_MRT_MICS3 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Mauritania MICS 2007 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Mauritania MICS 2007 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/MRT_MICS3_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/MRT_MICS3_hh_dict.xlsx")
  ...
  data$region <- tolower(as.character(as_factor(mics$HH6AW)))
  data$prov <- NA
  data$dist <- NA
  ...
  data$mar_status <- ifelse(mics$mstatus==1, "currently", 
                            ifelse(mics$mstatus==2|mics$mstatus==3, "formerly", 
                                   ifelse(mics$mstatus==4, "never", NA)))
