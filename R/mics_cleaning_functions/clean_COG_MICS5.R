...
clean_COG_MICS5 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Congo_MICS5_Datasets/Congo MICS 2014-15 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Congo_MICS5_Datasets/Congo MICS 2014-15 SPSS Datasets/hh.sav")
  bh <- read_sav("data/mics/raw_data/Congo_MICS5_Datasets/Congo MICS 2014-15 SPSS Datasets/bh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/COG_MICS5_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/COG_MICS5_hh_dict.xlsx")
  write_xlsx(bh.dict, "data/mics/data_dictionaries/COG_MICS5_bh_dict.xlsx")
  ...
  data$relig <- ifelse(data$relig=="non déclarée", NA, data$relig)
  data$relig <- ifelse(data$relig=="ne sait pas", NA, data$relig)
  data$relig <- ifelse(data$relig=="autre", "other religion", data$relig)
  data$relig <- ifelse(data$relig=="catholique", "catholic", data$relig)
  data$relig <- ifelse(data$relig=="eglises de réveil", "pentecostal", data$relig)
  data$relig <- ifelse(data$relig=="kimbaguiste", "kimbanguist", data$relig)
  data$relig <- ifelse(data$relig=="musulman", "muslim", data$relig)
  data$relig <- ifelse(data$relig=="protestante", "protestant", data$relig)
  data$relig <- ifelse(data$relig=="salutiste", "salvationist", data$relig)
  ...
  data$ethn <- ifelse(data$ethn=="non déclarée"|data$ethn=="ne sait pas", NA, data$ethn)
  ...
  # Marriage age
  data$mar_age <- as.numeric(mics$WAGEM)
