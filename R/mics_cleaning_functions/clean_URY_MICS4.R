...
clean_URY_MICS4 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Uruguay_MICS4_Datasets/Uruguay MICS 2012-13 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Uruguay_MICS4_Datasets/Uruguay MICS 2012-13 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/URY_MICS4_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/URY_MICS4_hh_dict.xlsx")
  ...
  summary(data$psu)
  data$strata <- as.numeric(mics$strat)
  summary(data$strata)
  
  # Sub-national region names
  data$region <- ifelse(mics$region==1, "Montevideo y Zona Metropolitana", 
                     ifelse(mics$region==2, "Norte", 
                            ifelse(mics$region==3, "Costa Este",
                                   ifelse(mics$region==4,"Litoral Sur",
                                          ifelse(mics$region==5, "Centro",
                                                 ifelse(mics$region==6, "Centro Sur",
                                                        ifelse(mics$region==7, "Litoral Norte", NA)))))))
  data$region <- as.factor(data$region)
  data$prov <- NA
  data$dist <- NA
  
  # Rural-urban place of residence
  data$res <- ifelse(mics$HH6a==1, "urban", ifelse(mics$HH6a==2, "rural", NA))
  ...
  data$ethn <- ifelse(data$ethn=="sin dato", NA, data$ethn)
