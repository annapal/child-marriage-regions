...
clean_GUY_MICS3 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Guyana MICS 2006-07 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Guyana MICS 2006-07 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/GUY_MICS3_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/GUY_MICS3_hh_dict.xlsx")
  ...
  data$region <- tolower(as.character(as_factor(mics$WM1A)))
  data$prov <- NA
  data$dist <- NA
  ...
  data$ethn <- ifelse(data$ethn=="ns/missing", NA, data$ethn)
  data$ethn <- ifelse(data$ethn=="don't know", NA, data$ethn)
