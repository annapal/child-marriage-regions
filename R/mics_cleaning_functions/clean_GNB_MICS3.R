...
clean_GNB_MICS3 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Guinea-Bissau MICS 2006 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Guinea-Bissau MICS 2006 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/GNB_MICS3_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/GNB_MICS3_hh_dict.xlsx")
  ...
  data$relig <- ifelse(data$relig=="autre chretien", "other christian religion", data$relig)
  data$relig <- ifelse(data$relig=="autre religion", "other religion", data$relig)
  data$relig <- ifelse(data$relig=="chretien,catholique", "catholic", data$relig)
  ...
  data$ethn <- ifelse(data$ethn=="sans ethnie", "no ethnicity", data$ethn)
