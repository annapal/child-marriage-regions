...
clean_TTO_MICS3 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Trinidad and Tobago MICS 2006 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Trinidad and Tobago MICS 2006 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/TTO_MICS3_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/TTO_MICS3_hh_dict.xlsx")
  ...
  # Cluster number
  data$clust <- as.numeric(mics$WM1A)
  summary(data$clust)
  ...
  # Religion
  data$relig <- tolower(as.character(as_factor(mics$hhrel)))
  data$relig <- ifelse(data$relig=="no religion/dk/missing", NA, data$relig)
  data$relig <- ifelse(data$relig=="other", "other religion", data$relig)
  table(data$relig, useNA = "always")
  
  # Ethnicity
  data$ethn <- tolower(as.character(as_factor(mics$hheth)))
