...
clean_CRI_MICS6 <- function(survey_var) {
  
  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Costa Rica MICS6 Datasets/Costa Rica MICS6 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Costa Rica MICS6 Datasets/Costa Rica MICS6 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/CRI_MICS6_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/CRI_MICS6_hh_dict.xlsx")
  ...
  data$wt <- as.numeric(mics$zwmweight)
  summary(data$wt)
  ...
  data$relig <- ifelse(data$relig=="no sabe, no responde", NA, data$relig)
  table(data$relig, useNA = "always")
  
  # Ethnicity
  data$ethn <- tolower(as.character(as_factor(mics$HC2)))
  data$ethn <- ifelse(data$ethn=="ns, no responde", NA, data$ethn)
