...
clean_PAK_MICS4_Punjab <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Pakistan (Punjab)_MICS4_Datasets/Pakistan (Punjab) MICS 2011 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Pakistan (Punjab)_MICS4_Datasets/Pakistan (Punjab) MICS 2011 SPSS Datasets/hh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/PAK_MICS4_Punjab_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/PAK_MICS4_Punjab_hh_dict.xlsx")
  ...
  data$survey <- "MICS"
  data$phase <- 4
  data$surveyid <- "PAK_MICS4_Punjab"
  ...
  data$region <- tolower(as.character(as_factor(mics$div)))
  data$prov <- NA
  data$dist <- NA
  ...
  table(data$ethn, useNA = "always") # Missing values OK. Sometimes missing for entire sample
