...
clean_TCD_MICS6 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Chad MICS6 Datasets/Chad MICS6 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Chad MICS6 Datasets/Chad MICS6 SPSS Datasets/hh.sav")
  bh <- read_sav("data/mics/raw_data/Chad MICS6 Datasets/Chad MICS6 SPSS Datasets/bh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/TCD_MICS6_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/TCD_MICS6_hh_dict.xlsx")
  write_xlsx(bh.dict, "data/mics/data_dictionaries/TCD_MICS6_bh_dict.xlsx")
  ...
  data$relig <- ifelse(data$relig=="autres religions (? preciser)", "other religion", data$relig)
  ...
  data$ethn <- ifelse(data$ethn=="non reponse", NA, data$ethn)
  ...
  data$prev_reg <- ifelse(data$prev_reg=="En dehors du Tchad", "international", data$prev_reg)
  table(data$prev_reg, useNA = "always")
  
  # Marriage information ------------------------------------------
  
  # Current marriage status
  data$mar_status <- ifelse(mics$MSTATUS==1|mics$MSTATUS==2, "currently", 
