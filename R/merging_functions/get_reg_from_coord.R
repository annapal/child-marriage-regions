...
get_reg_from_coord <- function(data, iso3) {
  ...
  # Get Adm1 and Adm2 regions from coordinates in data
  ...
  # Get GADM regions
  adm1 <- gadm(iso3, level=1, path=paste0(getwd(), "/ref_data"), version="3.6")
  ...
  # Get GADM regions
  adm2 <- gadm(iso3, level=2, path=paste0(getwd(), "/ref_data"), version="3.6")
