...
clean_PRY_MICS5 <- function(survey_var) {

  # Import MICS data files
  wm <- read_sav("data/mics/raw_data/Paraguay_MICS5_Datasets/Paraguay MICS 2016 SPSS Datasets/wm.sav")
  hh <- read_sav("data/mics/raw_data/Paraguay_MICS5_Datasets/Paraguay MICS 2016 SPSS Datasets/hh.sav")
  bh <- read_sav("data/mics/raw_data/Paraguay_MICS5_Datasets/Paraguay MICS 2016 SPSS Datasets/bh.sav")
  ...
  write_xlsx(wm.dict, "data/mics/data_dictionaries/PRY_MICS5_wm_dict.xlsx")
  write_xlsx(hh.dict, "data/mics/data_dictionaries/PRY_MICS5_hh_dict.xlsx")
  write_xlsx(bh.dict, "data/mics/data_dictionaries/PRY_MICS5_bh_dict.xlsx")
  ...
  data$psu <- data$clust
  data$strata <- as.numeric(mics$strata2)
  summary(data$strata)
  ...
  data$relig <- ifelse(data$relig=="no reportado", NA, data$relig)
  data$relig <- ifelse(data$relig=="catolica", "catholic", data$relig)
  data$relig <- ifelse(data$relig=="evangélica", "evangelical", data$relig)
  data$relig <- ifelse(data$relig=="mormón", "mormon", data$relig)
  data$relig <- ifelse(data$relig=="ninguna religión", "no religion", data$relig)
  data$relig <- ifelse(data$relig=="otra religion", "other religion", data$relig)
  ...
  # Ever had sex
  data$sex_ev <- ifelse(mics$SB1>=97, NA, 
                        ifelse(mics$SB1==0, 0, 
  ...
  data$sex_age <- ifelse((data$sex_age>=97)|(data$sex_age==0), NA, 
